...
export class FileUpload extends Middleware {
    ...
    async handle(req, next) {
        ...
        if (this.maxFiles !== undefined && parsed.files.length > this.maxFiles) {
            return Response.json({ error: `Too many files. Maximum allowed: ${this.maxFiles}` }, { status: 413 });
        }
        for (const file of parsed.files) {
            if (file.size > this.maxSize) {
                return Response.json({ error: `File "${file.fileName}" exceeds maximum size of ${this.maxSize} bytes` }, { status: 413 });
            }
            if (this.allowedTypes && this.allowedTypes.length > 0) {
                const allowed = this.allowedTypes.some((pattern) => matchesMimeType(file.type, pattern));
                if (!allowed) {
                    return Response.json({ error: `File type "${file.type}" is not allowed` }, { status: 415 });
                }
            }
        }
